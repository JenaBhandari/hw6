...
{
	//cout << "in insert" << endl;
	//cout << "total with delete: " << totalWDelete_ << " total without: " <<totalItems_ << endl;
	//cout << "alpha: " << alpha << " capacity: " << CAPACITIES[mIndex_]  << endl;
	//cout << "alpha * capacity: " << alpha * CAPACITIES[mIndex_] << endl;
	if (totalWDelete_ >= (alpha * CAPACITIES[mIndex_]))
	{
	...
	HASH_INDEX_T theIndex = probe(newItem->item.first);
	////cout << "item: " << "key: " << newItem->item.first << " index: " << theIndex << endl;
	...
	{
		//if replacing value of key already there
		if (table_[theIndex] != nullptr)
		{
			totalItems_--;
			totalWDelete_--;
		}

		table_[theIndex] = newItem;
		totalItems_++;
		totalWDelete_++;
	}	

	//cout << "after insert\ntotal with delete: " << totalWDelete_ << " total without: " <<totalItems_ << endl;
...
{
	//cout << "in delete" << endl;
	HashItem* theItem = internalFind(key);
	if (theItem != nullptr)
	{
		//cout << "in found item: " << theItem->item.first << endl;
		theItem->deleted = true;
		totalItems_--; 
		//continue to count toward the loading factor until the next resize/rehash
		//totalWDelete_ stays same
	}
	else
	{
		return;
...
{
 	// check if there are more capacities to resize to
  if (mIndex_ + 1 >= sizeof(CAPACITIES)) 
	{
    throw std::logic_error("At largest capacity");
  }
  ...
  mIndex_ = mIndex_ + 1;
	...
	// << "old capacity: " << oldCapacity << endl;
	//cout << "new capacity: " << newCapacity << endl;
	
	

	std::vector<HashItem*> oldTable;
	oldTable.resize(oldCapacity, nullptr);
	//cout << "created oldtable" << endl;
	...
	for (size_t i = 0; i < oldCapacity; ++i) 
	{
		...
		if (table_[i] != nullptr)
		{
			//cout << "table_[" << i << "]: " << table_[i]->item.first << endl;
			//cout << "item deleted? " << (table_[i]->deleted) << endl;
			HashItem *item = table_[i];
			if ( (item != nullptr) && (item->deleted == false) ) 
			{
				//cout << "assign to old table" << endl;
				oldTable[i] = item;
			} 
			else 
			{
			...
			//cout << "oldTable_[" << i << "]: " << oldTable[i]->item.first << endl;
			table_[i] = nullptr;
	...
	//cout << "before resized table" << endl;
	table_.resize(newCapacity, nullptr);
	//cout << "after resized table" << endl;
	...
	for (size_t i = 0; i < oldCapacity; ++i) 
	{
		if (oldTable[i] != nullptr)
		{
			...
			// probe for an available slot in the new table
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    {
				//cout << "table_[" << loc << "]: " << table_[loc] << endl;
				//cout << (nullptr == table_[loc]) << endl;
